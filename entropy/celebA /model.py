...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, is_crop=True,
    ...
    self.i_bn1 = batch_norm(name='i_bn1')
    self.i_bn2 = batch_norm(name='i_bn2')
    self.i_bn3 = batch_norm(name='i_bn3')
  ...
  def build_model(self):
    ...
    # Placeholders
    self.anneal_step=tf.Variable(0,trainable=False)
    self.anneal_step_op=self.anneal_step.assign_add(1)
    ...
    self.image_noise=tf.random_normal([self.sample_num]+image_dims[:2]+[1])
    ...
    if self.y_dim:
      ...
      self.z_rec = self.inference(self.G,self.image_noise)
    ...
    self.d_loss_real = tf.reduce_mean(
      sigmoid_cross_entropy_with_logits(self.D_logits_real, tf.zeros_like(self.D_real)))
    # Discriminator Loss Fake
    self.d_loss_fake = tf.reduce_mean(
      sigmoid_cross_entropy_with_logits(self.D_logits_fake, tf.ones_like(self.D_fake)))

    # Reconstruction Loss
    self.cost_z = tf.reduce_mean(tf.pow(self.z_rec - self.z, 2))

    # Generator Loss
    beta=1.0-tf.minimum(tf.cast(self.anneal_step,tf.float32)/100000,1.)
    print(beta)
    self.g_loss = tf.reduce_mean(self.D_logits_fake) + beta*self.cost_z
    ...
    # Generator
    grads_and_vars = opt_g.compute_gradients(self.g_loss, var_list=self.g_vars  + self.i_vars)
  ...
  def train(self, config):
    ...
    for epoch in range(config.epoch):
      ...
      for batch_idx in range(batch_nums):
        ...
        counter += 1
        self.sess.run(self.anneal_step_op)
  # Discriminator
  ...
  # Inference
  def inference(self, image,noise, y=None, reuse=False):
    with tf.variable_scope("inference") as scope:
      if reuse:
        scope.reuse_variables()
      input=tf.concat((image,noise),3)
      print('adding noise')
      h0 = lrelu(conv2d(image, self.df_dim, name='i_h0_conv'))
      h1 = lrelu(self.i_bn1(conv2d(h0, self.df_dim*2, name='i_h1_conv')))
      h2 = lrelu(self.i_bn2(conv2d(h1, self.df_dim*4, name='i_h2_conv')))
      h3 = lrelu(self.i_bn3(conv2d(h2, self.df_dim*8, name='i_h3_conv')))
      h4 = linear(tf.reshape(h3, [self.batch_size, -1]), self.z_dim, 'i_h3_lin')
      return h4
