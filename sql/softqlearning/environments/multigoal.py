...
class MultiGoalEnv(Env, Serializable):
    ...
    def __init__(self, goal_reward=10, actuation_cost_coeff=30,
        ...
        #
        # array = np.arange(-2.5, 2.5, 0.1)
        # a1 = np.vstack([array, np.ones_like(array) * 5]).T
        # a2 = np.vstack([array, -np.ones_like(array) * 5]).T
        # a3 = np.vstack([np.ones_like(array) * 5, array]).T
        # a4 = np.vstack([-np.ones_like(array) * 5, array]).T
        # self.goal_positions = np.concatenate([a1, a2, a3, a4], axis=0).astype('float32')
    ...
    def render(self, paths):
        ...
        plt.draw()
        plt.pause(0.01)
        self.out_dir = logger._snapshot_dir
        plt.savefig('{}/{}'.format(self.out_dir,strftime("%Y-%m-%d-%H:%M:%S", gmtime())+'.png'))
